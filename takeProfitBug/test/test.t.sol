...
contract WalterTest is Test {
    ...
    function stringToUint(string memory s) public pure returns (uint256 result) {
     ...
     function test_userLoseValueDueToClosingAtBid()external{
        vm.startPrank(bob);
        vm.recordLogs();
        ...
        vm.recordLogs();
        vm.prank(TRADES_UPKEEP);
        trading.executeAutomationOrder(Trading.LimitOrder.TP,bob,0,0,block.timestamp);
        logs = vm.getRecordedLogs();
        orderId = uint256(logs[1].topics[1]);
        ...
        upKeepAnswer = TradingCallback.PriceUpKeepAnswer(
            ...
            102e18,                     // Market price
            100e18-1,                   // Bid (buying)
            102e18+1,                   // Ask (selling)      
            true                       // Trading closed
        );

        tc.executeAutomationCloseOrderCallback(upKeepAnswer);
        vm.stopPrank();

        console.log("balance when bid is at $100 and spot is 102$: ",IERC20(usdc).balanceOf(bob)," USDC");
        console.log("resulting in market mode close even if it is limit order");
    }

    function test_bobGetsBibPriceCloseToMarket()external{
        vm.startPrank(bob);
        vm.recordLogs();
        Trading.OpenOrderType orderType = Trading.OpenOrderType.MARKET;
        Trading.Trade memory trade = Trading.Trade(
        ...
        console.log("balance when bid is at $102 and spot is 102$: ",IERC20(usdc).balanceOf(bob)," USDC");
    }
}
interface PairInfos{

...
contract WalterTest is Test {
    ...
    function stringToUint(string memory s) public pure returns (uint256 result) {
     ...
     function test_InsolventUserRemoveCollateral()external{
        ...
        uint256 balanceBefore = ERC20(usdc).balanceOf(bob);
        (uint256 collateralBefore,,,,,,,,) = storageT.openTrades(bob, 0, 0);
        require(collateralBefore!=0);

        vm.startPrank(bob);
        vm.recordLogs();
        trading.removeCollateral(0, 0, 15000e6);

        logs = vm.getRecordedLogs();
        orderId = uint256(logs[0].topics[1]);
        vm.stopPrank();

        (int256 percentageProfit,) = _currentPercentProfit(100e18,72e18,true,300,300);
        console.log("the position is in negative by: ",percentageProfit); // around -84%
        
        vm.prank(PRICE_UPKEEP);
        upKeepAnswer = TradingCallback.PriceUpKeepAnswer(
            orderId,                    // Order trade id
            72e18,                     // Market price
            72e18-1,                   // Bid (buying)
            72e18+1,                   // Ask (selling)      
        ...
        tc.handleRemoveCollateral(upKeepAnswer);

        uint256 balanceAfter = ERC20(usdc).balanceOf(bob);
        (uint256 collateralAfter,,,,,,,,) = storageT.openTrades(bob, 0, 0);
        
        // DIFFERENT CHECKS
        vm.assertGt(balanceAfter,balanceBefore);
        vm.assertGt(collateralBefore,collateralAfter);
    }
